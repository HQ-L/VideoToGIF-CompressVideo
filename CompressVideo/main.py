...
import time
import cv2
convert_cmd = 'ffmpeg -threads 4 -i ./originPath/src_video_name -vcodec libx264 -preset fast -crf 24 -y -vf "scale=1920:-1" -acodec libmp3lame -ab 32k ./completionPath/dest_video_name'
...
def transform():
    ...
    for (_, _, subfile) in os.walk(root_to):
        for f in subfile:
            if f.find(".mp4") > -1:
